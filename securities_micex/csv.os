...
Функция НайтиВсеСтроки(ПолноеИмяФайла)
	...
	СпецСимволы = "~`'!""№,\.<>;%:\?\(\)\+@#$\^\&\/\*\-\\\{\}\[\]\|";
	ВсеСимволы = "[A-Za-z0-9_]";

	к = Символ(34);
	//ШаблонПоля = к + ".*?" + к;

	//ШаблонПоля = к + "[\w\s'!""№,\.<>;%:\?\(\)\+@#$\^\&\/\*\-\\\{\}\[\]\|]*?" + к;
	ШаблонПоля = к +"{1}"+ "[="+символ(34)+",\-+;\*:\w\s\\\.\(\)\?\/]*" + к+"{1}";

	ШаблонСтроки = "^\w\W*$";
	Шаблон = "^";
	Для сч = 1 по 38 Цикл Шаблон = Шаблон + ШаблонПоля + Разделитель ; КонецЦикла;
	//последний разделитель может быть, а может и не быть
	//Шаблон = Шаблон + ШаблонПоля + Разделитель + "{0,1}"  ;
	Шаблон = Шаблон + ШаблонПоля + Разделитель + "?";
	//Сообщить(Шаблон);

	РегЭксп = Новый Regex(ШаблонСтроки); 
	РегЭксп.IgnoreCase = Истина;
	РегЭксп.Multiline = Истина;

	РегЭкспПоля = Новый Regex("" + символ(34) + ";" + символ(34)+ "{0,1}");
	РегЭкспПоля.IgnoreCase = Истина;
	РегЭкспПоля.Multiline = Ложь;
	
	Для сч = 2 по т.количествоСтрок() Цикл
	
		Поля = РегЭкспПоля.НайтиСовпадения(т.ПолучитьСтроку(сч)); 
		///Сообщить("Полей = " + Строка(Поля.Количество()));
		для каждого Поле из Поля цикл
			//Сообщить(Поле.группы.Количество());
			для каждого Группа из Поле.группы цикл
				//Сообщить(группа.значение);
			КонецЦикла;
		КонецЦикла;

		мСтрок = РегЭкспПоля.Разделить(т.ПолучитьСтроку(сч)); 
		Для сч2 = -мСтрок.Количество()+1 по 0 цикл
			если мСтрок[-сч2] = """;""" Тогда
				мСтрок.Удалить(-сч2);
			КонецЕсли;
		конецЦикла;
		для сч2 = 0 по мСтрок.Количество()-1 Цикл
			//сообщить(мСтрок[сч2]);
		конецЦикла;
		сообщить(мСтрок.Количество());
	КонецЦикла;
